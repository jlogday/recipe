...
public class LocationHandler {
    ...
    public Mono<ServerResponse> getLocationById(ServerRequest request) {
        int id = Integer.parseInt(request.pathVariable("id"));
        ObjectMapper mapper = new ObjectMapper();
        record Location(int id, String name, String city, String state, String photo, int availableUnits, boolean wifi, boolean laundry) {}
        var type = mapper.getTypeFactory().constructCollectionType(List.class, Location.class);
        try {
            List<Location> list = mapper.readValue(JSON, type);
            log.debug("list: {}", list);
            Location location = list.stream()
                    .filter(loc -> loc.id == id)
                    .findFirst()
                    .orElseThrow();

            return ServerResponse.ok().contentType(MediaType.APPLICATION_JSON).bodyValue(location);
